      ...
      body: Column(
        children: [
          ...
          BlocBuilder<MainBloc, MainState>(
            builder: (context, state) {
              List<ToCelda> titles = state.wbe!.titles;
              return Row(
                children: [
          ...
          Expanded(
            child: BlocBuilder<MainBloc, MainState>(
              builder: (context, state) {
                if (state.wbe == null || firstTimeLoading) {
                  return const Center(
                ...
                wbeList = wbeList
                    ...
                    .toList();
                if (wbeList.length > endList) {
                  wbeList = wbeList.sublist(0, endList);
                }
                return SingleChildScrollView(
                  ...
                  child: SelectableRegion(
                    ...
                    child: Column(
                      children: [
                        for (WbeSingle wbe in wbeList)
                          Builder(builder: (context) {
                            return Row(
                              children: [
                                for (ToCelda celda in wbe.celdas)
                                  Expanded(
                                    flex: celda.flex,
                                    child: Text(
                                      celda.valor,

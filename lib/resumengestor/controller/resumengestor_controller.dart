...
class ResGestorController {
  ...
  void crear() {
    try {
      ResumenGestor resumenGestor = ResumenGestor();
      List<ResumenGestorSingle> list = resumenGestor.resumenGestorList;
      ...
      for (SegOda segOda in segOdasFiltered) {
        ...
        ResumenGestorSingle? resumen = list.firstWhereOrNull((e) =>
            e.gestor == segOda.gestor && e.proveedor == segOda.proveedor);
            
        // Handle case when resumen is null by creating a new entry
        if (resumen == null) {
          resumen = ResumenGestorSingle(
            gestor: segOda.gestor,
      ...
      resumenGestor.resumenGestorList =
          list.where((e) => e.total != 0).toList();

      // print('list: $list');
      emit(state().copyWith(resumenGestor: resumenGestor));
    } catch (e) {
      bl.errorCarga('Resumen Gestor', e);
    }
  }
}
